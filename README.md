...
## Research 047 — statement versus press conference

[Issue #64](https://github.com/HarryWarre/trading-model-ai-lab/issues/64) separates the paper-defined FOMC statement and press-conference windows. An expanding, prior-event-only ridge with economically defined cross-asset inputs returns +1.7987% after the fixed four-pip round trip across 12 held-out meetings, versus +0.3750% for continuation, -1.1553% for reversal and -0.3853% for long-only. It abstains on 69.72% of possible asset-events and remains +1.3076% at 4x cost.

The model nevertheless fails the preregistered decision: 2025 is -0.4356%, and the paired two-event-block resample fraction that it beats continuation is 79.65%, below 95%. This is research-only, not alpha. Do not tune the window, ridge penalty, feature set or abstention threshold on these outcomes. Highest-value next step is timing-audited official policy-surprise data or a genuinely later untouched meeting sample; Research 039 remains blocked without point-in-time consensus data. See [report](https://github.com/HarryWarre/trading-model-ai-lab/blob/main/research/real_fomc_statement_pressconference_2023_2025.md) and [results](https://github.com/HarryWarre/trading-model-ai-lab/tree/main/results/research047).


## Research 048 — official statement surprise

[Issue #65](https://github.com/HarryWarre/trading-model-ai-lab/issues/65) uses the official SF Fed USMPD statement-window rate changes available at T+20 to forecast the subsequent press-conference window across the validated CFD panel. The expanding Ridge returned +1.2715% at the four-pip assumption, remained positive at 4x cost, and survived every asset/family omission, but failed the locked decision: 2025 was -0.8391% and the paired probability of beating price-only was 69.28%, below 95%. Ridge is rejected/research-only.

The preregistered conventional-policy economic baseline returned +1.8015% at 1x and +1.1805% at 4x, was positive in both 2024 and 2025, and survived every asset/family omission. Because the primary gated model failed and this sample has already been inspected, the baseline is **not** promoted. Highest-priority next work is an unchanged test on a comparable untouched 2026+ 15-asset CFD panel. Official USMPD already covers six 2026 meetings; comparable CFD prices are the blocker. See the [report](https://github.com/HarryWarre/trading-model-ai-lab/blob/main/research/real_usmpd_statement_pressconference_2023_2025.md) and [results](https://github.com/HarryWarre/trading-model-ai-lab/tree/main/results/research048).


## Research 049 — post-selection audit of the economic rule
